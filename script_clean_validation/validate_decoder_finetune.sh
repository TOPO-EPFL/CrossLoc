...
if [[ "${CKPT_DIR}" == *"-pairwise-oop-"* ]];
  then
    SECTION_NM=val_oop_drone_real
    echo "Out-of-place validation set is used."
  else
    ...
    case $NET_DEPTH in
      "TINY")
        python3 test_single_task.py ${DATASET} --task ${TASK} --uncertainty ${UNC} --section ${SECTION_NM} \
          ...
          --keywords ${DATASET} ${TASK} -tiny decoder_coord_free_depth_normal
        ;;
      "FULL")
        python3 test_single_task.py ${DATASET} --task ${TASK} --uncertainty ${UNC} --section ${SECTION_NM} \
          --network_in ${CKPT_DIR} --min_ckpt_iter ${MIN_CKPT_ITER} --max_ckpt_iter ${MAX_CKPT_ITER} \
          --keywords ${DATASET} ${TASK} decoder_coord_free_depth_normal
        ;;
      *)
