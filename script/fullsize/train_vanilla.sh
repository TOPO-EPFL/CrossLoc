...
case $TASK in
  "coord")
    case $NET_DEPTH in
      "TINY")
        python3 train_single_task.py ${DATASET} --task ${TASK} --supercon_weight 0.0 \
          --learningrate ${LR} --epochs ${EPOCHS} --inittolerance ${SCR_TOL}  --batch_size ${BATCH_SIZE} \
          --uncertainty --fullsize --auto_resume --tiny --ckpt_dir ${CKPT_DIR} --real_chunk ${REAL_CHUNK}
        ;;
      "FULL")
        python3 train_single_task.py ${DATASET} --task ${TASK} --supercon_weight 0.0 \
          --learningrate ${LR} --epochs ${EPOCHS} --inittolerance ${SCR_TOL}  --batch_size ${BATCH_SIZE} \
          --uncertainty --fullsize --auto_resume --ckpt_dir ${CKPT_DIR} --real_chunk ${REAL_CHUNK}
        ;;
      *)
