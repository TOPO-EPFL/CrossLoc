...
ENC_COORD=$PROJ_DIR/weights-clean/encoders-pretraining/${DATASET}-pairwise-only/${PT_REAL_DATA_DOM}/coord/model-sc-${PT_SIM_DATA_CHUNK}-rc-${PT_REAL_DATA_CHUNK}.net
ENC_DEPTH=$PROJ_DIR/weights-clean/encoders-pretraining/${DATASET}-pairwise-only/${PT_REAL_DATA_DOM}/depth/model-sc-${PT_SIM_DATA_CHUNK}-rc-${PT_REAL_DATA_CHUNK}.net
ENC_NORMAL=$PROJ_DIR/weights-clean/encoders-pretraining/${DATASET}-pairwise-only/${PT_REAL_DATA_DOM}/normal/model-sc-${PT_SIM_DATA_CHUNK}-rc-${PT_REAL_DATA_CHUNK}.net
ENC_SEMANTICS=$PROJ_DIR/weights-clean/encoders-pretraining/${DATASET}-pairwise-only/${PT_REAL_DATA_DOM}/semantics/model-sc-${PT_SIM_DATA_CHUNK}-rc-${PT_REAL_DATA_CHUNK}.net

SP_SESSION=''
if [ "$PT_REAL_DATA_DOM" == "in_place" ];
  then
    SP_SESSION='enc-pairwise-only-pt'$PT_SIM_DATA_CHUNK-ip-ft${PT_REAL_DATA_CHUNK}
  else
    SP_SESSION='enc-pairwise-only-pt'$PT_SIM_DATA_CHUNK-oop-ft${PT_REAL_DATA_CHUNK}
fi
